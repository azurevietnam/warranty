...
<form action="<?php echo JRoute::_('index.php?option=com_warranty&view=specs'); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
		...
		<table class="table table-striped" id="productList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_WARRANTY_PRODUCT', 'product_title', $listDirn, $listOrder); ?>
					</th>
                    <th>
                        <?php echo JText::_('COM_WARRANTY_IMAGE')?>
                    </th>
                    <th>
                        <?php echo JText::_('COM_WARRANTY_PRICE')?>
                    </th>
                    <th>
                        <?php echo JText::_('COM_WARRANTY_TITLE_SPEC')?>
                    </th>
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>">
                    ...
                    <td><a href="<?php echo JRoute::_('index.php?option=com_warranty&task=spec.edit&id='.(int) $item->id); ?>"><?php echo $item->product_title; ?></a></td>
                    <td>
                        <?php if($item->images && $item->images->get('image_intro')){?>
                            <a href="<?php echo JUri::root().$item->images->get('image_intro');?>" class="modal">
                            <img src="<?php echo JUri::root().$item->images->get('image_intro');?>" alt="<?php echo $item->images->get('image_intro_alt');?>" style="height: 100px"/>
                            </a>
                        <?php }?>
                    </td>
                    <td>
                        <?php if($item->attribs) echo $item->attribs->get('title_caption');?>
                    </td>
                    <td>
                        <ul>
                        <?php
                        for($i=1;$i<=10;$i++){
                            $field = 'field'.$i;
                            if($item->$field) echo '<li>'.$item->$field.'</li>';
                        }
                        ?>
                        </ul>
                    </td>
