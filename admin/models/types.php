 ...
 */
class WarrantyModelTypes extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array(
                ...
                'code', 'a.code',
                'name', 'a.name',
                'product', 'a.product', 'product_title'
            );
        }
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__warranty_models` AS a');

        $query->select('c.title AS product_title, c.images, c.attribs');
        $query->join('LEFT', '#__content c ON c.id = a.product');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('(a.code LIKE '.$search.' OR a.name LIKE '.$search.' OR c.title LIKE '.$search.')');
            }
        }
